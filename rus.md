...
рендеринга и сборки страниц в Chrome с использованием графического ускорителя (GPU).
Например, [«Ускоренный рендеринг в Chrome: Многоуровневая модель»][2] — это
отличное введение в то, как Chrome использует понятие слоя для отрисовки
...
управляет растеризацией веб-страницы, но и используется для удовлетворения
всех потребностей HTML5 Canvas API (блоки, линии, растровые заливки, тени, размытия,
все вызовы Chrome для превращения DOM в пиксели). Для облегчения отладки
...
Используя этот пакет, мы генерируем набор HTML-страниц, каждая из которых 
содержит уникальную комбинацию CSS-свойств и их значений, например:
...
Далее, интересно то, что **в комбинации CSS-свойства могут иметь большее 
время отрисовки, нежели в сумме по отдельности**. С точки зрения наблюдателя
...
- Если сомневаетесь, всегда склоняйтесь в сторону решения, обеспечивающего
...
отрисовка свойств ускорится). «Браузерное» ПО — это постоянно изменяющаяся
...
чтобы определить насколько они вам подходят.
